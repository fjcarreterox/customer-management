...
foreach($files as $f){
    ...
    $q = "SELECT * FROM rel_estructuras WHERE `idfichero` =".$f['id'];
    $sdata = DB::query($q)->as_assoc()->execute();

    foreach($sdata as $sd){
        $q = "SELECT * FROM tipo_datos WHERE `id` =".$sd["idtipodato"];
        $datatype = DB::query($q)->as_assoc()->execute()[0];
...
if(count($trab)>0){
    foreach($trab as $t) {
        ...
        $pdf->Row(array(html_entity_decode($t["nombre"]), $t["cargofuncion"], $falta, $fbaja));
    }
}
else{
    $pdf->Row(array("","","",""));
...
foreach($ces as $c){
    $ces_name="N/D";
    if($c["idcesionaria"]!=0){
        $q = "SELECT * FROM clientes WHERE `id` =".$c["idcesionaria"];
        $ces_name = DB::query($q)->as_assoc()->execute()[0]["nombre"];
    }
    $ces_cif = DB::query($q)->as_assoc()->execute()[0]["cif_nif"];
